# Batch runner for nirs_individual_pipeline.Rmd: render every NIRS-eligible run one at a time,
# logging success/error per run. Eligibility comes from the subset registry's nirs_individual
# block (included == 1). Run from the project root.
library(arrow)
library(dplyr)
library(rmarkdown)
...
report_dir <- here::here("results", "QC_EXPORT", "nirs_individual_analysis")
analysis_block <- "nirs_individual"  # registry block that decides which runs are eligible
...
for (run_id in valid_run_ids) {
  ...
  render_log[[run_id]] <- tryCatch({
    rmarkdown::render(
      input       = here::here("notebooks", "nirs_individual_pipeline.Rmd"),
      params      = list(RUN_ID = run_id),
      output_dir  = report_dir,
      output_file = paste0(run_id, "_nirs_report.html"),
      quiet       = TRUE
    )
    "success"
